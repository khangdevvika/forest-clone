...
export function Heatmap({ data, className }: HeatmapProps) {
  ...
  const getColorClass = (level: number) => {
    switch (level) {
      case 0: return "bg-sage-100/40 dark:bg-white/5"
      case 1: return "bg-sage-200"
      case 2: return "bg-sage-400"
      case 3: return "bg-sage-600"
      case 4: return "bg-sage-700"
      ...
      <CardHeader className="flex flex-row items-center justify-between px-8 pt-8 pb-4">
        <div className="space-y-1">
          ...
          <CardDescription className="text-[10px] uppercase tracking-widest font-bold opacity-60">
            Evolution of your digital forest
          </CardDescription>
        </div>
                    ...
                    {week.map((day) => {
                      ...
                      return (
                        <Tooltip key={dateStr}>
                          ...
                          >
                            <span className="text-[10px] font-medium font-(family-name:--font-inter) text-muted-foreground">
                              {format(day, 'MMM d')}
                            </span>
                            <span className="w-[1px] h-2.5 bg-foreground/10" />
                            <span className="text-[10px] font-bold font-(family-name:--font-outfit)">
